 ...
 <table style="padding: 5px;" width="100%">
 	<tr>
 		...
 		<td valign="bottom" align="right" width="50%">
 			<font size="3">
				&nbsp;&nbsp;<strong>欢迎：${userinfo.username}</strong>
				<a href="/doLoginOut">退出</a>
			</font>
 		</td>
		...
		<c:forEach items="${menulist}" var="item">
			<div title="${item.name}" data-options="selected:true,iconCls:'${item.icon}'" style="padding: 10px">
				<c:forEach items="${item.children}" var="child">
					<a href="javascript:openTab('${child.name}','${child.url}','${child.icon}')" class="easyui-linkbutton" data-options="plain:true,iconCls:'${child.icon}'" style="width: 150px">${child.name}</a>
				</c:forEach>
			</div>
		</c:forEach>
	</div>
