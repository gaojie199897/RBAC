...
<script type="text/javascript">
	...
	function openDeployAddDiglog(){
		...
		url="${pageContext.request.contextPath}/deploy/addDeploy.com.action"
	}
	function saveDeploy(){
		$("#fm").form("submit",{
			url:url,
	...
	function deleteDeploy(){
		...
		$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectRows.length+"</font>条数据吗?",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/deploy/delDeploy.com.action",{ids:ids},function(result){
					if(result.success){
...
<table id="dg" title="部署管理" class="easyui-datagrid"
  ...
  url="${pageContext.request.contextPath}/deploy/deployPage.com.action" fit="true" toolbar="#tb">
 <thead>
 	<tr>
